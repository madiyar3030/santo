                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table">
                                    ...
                                    @foreach($vaccines as $vaccine)
                                        <tr>
                                            <td>{{$vaccine->id}}</td>
                                            <td><img src="{{asset($vaccine->image)}}" alt="" style="max-width: 200px; max-height: 100px"></td>
                                            <td>
                                                {{!is_null($vaccine->share_file_url) ? 'Есть' : 'Нет'}}
                                            </td>
                                            <td>{{$vaccine->title}}</td>
                                            <td>{{$vaccine->description}}</td>
                                            <td>{{$vaccine->age_from}}</td>
                                            <td>{{$vaccine->age_to}}</td>
                                            <td>{{$vaccine->age_type}}</td>
                                            <td>
                                                <form id="detailsForm_{{$vaccine->id}}" action="{{route('details.index')}}">
                                                    <input type="hidden" name="type" value="vaccine">
                                                    <input type="hidden" name="detailable_id" value="{{$vaccine->id}}">
                                                    <a style="cursor: pointer;" onclick="document.getElementById('detailsForm_{{$vaccine->id}}').submit()" >Детали</a>
                                                </form>
                                            </td>
                                            <td style="min-width: 180px">
                                                <a href="{{route('vaccines.edit', $vaccine)}}" class="waves-effect btn bg-deep-orange"><i class="material-icons">edit</i></a>
                                                <form action="{{route('vaccines.destroy', $vaccine)}}" method="POST" style="display:inline-block">
                                                    @method('delete')
                                                    @csrf
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{route('vaccines.store')}}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group form-float p-t-5">
                                    <div class="form-line">
                                        <input type="number" step=0.1 type="text" class="form-control" id="age_from" name="age_from"/>
                                        <label class="form-label">Возраст от</label>
                                    </div>
                                </div>
                                <div class="form-group form-float p-t-5">
                                    <div class="form-line">
                                        <input type="number" step=0.1 type="text" class="form-control" id="age_to" name="age_to"/>
