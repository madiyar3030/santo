...
//            'pregnant' => 'required|boolean',
        ...
        if (!isset($user['email_verified_at'])) {
            $user->thumb = isset($request['image']) ? $this->upload($request['image']) : null;
            ...
            return $this->Result(400, null, 'Пользователь уже зарегистрирован');
        }
    ...
    public function addFavourites(Request $request) {
        ...
        $validator = $this->validator($request->all(), $rules);
        if ($validator->fails()) return $this->Result(400, null, $validator->errors()->first());
        $favourable = Favourable::where('favourable_type', $request['type'])
            ->where('favourable_id', $request['id'])
            ->where('user_id', $request['currentUser']->id)->first();
        if ($favourable) $favourable->delete();
        else {
            $favourable = new Favourable();
            $favourable->favourable_type = $request['type'];
            $favourable->favourable_id = $request['id'];
            $favourable->user_id = $request['currentUser']->id;
            $favourable->save();
        }
    ...
    public function createNotificationForVaccines(Request $request) {
        $blog = Blog::first();
        $res = Carbon::make($blog->online_from)->subHour()->format('H:i');
        $now = Carbon::now();
        $from = Carbon::make($blog->online_from);
        $to = Carbon::make($blog->online_to);
        $until_date = Carbon::make($blog->online_until);
        $asd = [
            'now' => $now,
        ...
        dump($asd);
        return \response()->json($now < $until_date && $now >= $from ?$now->diff($to)->format('%H:%I:%S') : null);
    }
